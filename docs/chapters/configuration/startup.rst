...
If you have jails that must be started before other jails, you can use
the priority or depend option. With the priority option Jails will start in
order starting at the lowest value, and will stop in order starting at the
highest value. So, jails with a priority value of 1 will start first, and stop last.

For the depend option, if a jail has any other jail specified, then Bastille will
first start the jail in the depend option, no matter what the priority is.
